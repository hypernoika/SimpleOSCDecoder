...
################################################################################
         ...
         >>> SimpleOSCReader Lazarus Windows Indy10 Unit Demo Program <<<
                                 Version 1.0
                              -------------------
...
NB1 : Don't forget to include the Indy library path into the project options.
...
uses
  ...
  Dialogs, Grids, StdCtrls, ExtCtrls, clipbrd, Menus, IdUDPServer,
  SimpleOSCDecoder, IdSocketHandle, IdGlobal;
...
begin
// Convert TIdBytes to TMemoryStream to keep INDY9 compatibility
ADataStream := TMemoryStream.Create;
ADataStream.Size := 0;
WriteTIdBytesToStream (ADataStream, AData);
ADataStream.Position := 0;
 // Check if the UDP Received data is not null and if the IdUDPServer is activated.
  if IdUDPServer.Active and assigned(ADataStream) and (ADataStream.Size > 0) and (ABinding.Handle >= 0)
    then begin
